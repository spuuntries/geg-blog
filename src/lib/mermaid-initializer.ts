// Google's Jules made this, dk what the diff is, but we're trying it out
  ...
  mermaidElements.forEach((element, index) => {
    try {
      ...
      if (rawDiagramDefinition) {
        ...
        console.log(
          `Prepared Mermaid diagram at index ${index} from data-content.`,
        )
        ...
        console.warn(
          `Mermaid element at index ${index} has no data-content and is empty. If it was previously rendered, mermaid.run() might attempt to re-render it if its content was set by an earlier call.`,
        )
      } else {
        ...
        console.log(
          `Mermaid element at index ${index} has no data-content, assuming it's already processed or will be handled by mermaid.run().`,
        )
      }
    } catch (error) {
      console.error(
        `Error processing Mermaid element at index ${index}:`,
        error,
      )
    }
  })
